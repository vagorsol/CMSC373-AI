...
% released_after(M, Y): movie M was released after year Y
released_after(M, Y):- movie(M, X), not(X=Y), X > Y.

% releasted_before(M, Y): movie M was released before year Y
released_before(M, Y):- movie(M, X), not(X=Y), X < Y.

% same_year(M1, M2): the two movies are released in the same year
same_year(M1, M2):- movie(M1, Y), movie(M2, Y), not(M1=M2).

% co_stars(A1, A2): the actor/actress are in the same movie
co_stars(A1, A2):- actor(M, A1, R1), actor(M, A2, R2), not(A1=A2).
co_stars(A1, A2):- actress(M, A1, R1), actress(M, A2, R2), not(A1=A2).
co_stars(A1, A2):- actress(M, A1, R1), actor(M, A2, R2), not(A1=A2).
co_stars(A1, A2):- actor(M, A1, R1), actress(M, A2, R2), not(A1=A2).
